...
def tool_module_path(tool: WebMCPTool) -> str:
    """Where the generated handler for this tool lives, repository-relative."""
    return f"{WEBMCP_DIR}/tools/{tool.handler_name}.ts"


def tool_test_path(tool: WebMCPTool) -> str:
    """Where this tool's generated test lives, repository-relative.

    Exported because F8-04's validator executes these files one at a time. A
    second copy of the path convention is a copy that drifts.
    """
    return f"{WEBMCP_DIR}/tools/{tool.handler_name}.test.ts"


REGISTER_PATH = f"{WEBMCP_DIR}/register.ts"
