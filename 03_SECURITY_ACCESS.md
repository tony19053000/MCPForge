...
- No outbound network from analysis and validation commands. Exactly **two** operations have the network, each in its own workspace, and nothing else ever runs in those workspaces:
  1. **The clone**, which targets only the bound repository host.
  2. **The dependency install** (`orchestration/dependencies.py`), added by the project owner's decision of 2026-09-11 so a connected repository's generated tools can actually be validated. Its constraints, each pinned by a test in `services/api/tests/test_dependency_install.py` unless stated:
     - *Its own networked workspace.* The install runs in a workspace created with the network, holding only the repository's `package.json` and `package-lock.json`, and its `node_modules` is then copied into the validation workspace. The validation workspace never has the network, and the validator refuses one that does (`test_the_install_runs_networked_and_validation_does_not`; `test_validation_refuses_a_workspace_with_network_access`; in the composed pipeline, `test_the_pr_leg_validates_on_dependencies_from_the_install_step`). A repository `.npmrc` is never copied in (`test_only_the_manifest_and_lockfile_reach_the_install_workspace`). Both files must be plain files: a link is refused before anything is staged, because `is_file()` follows a link and a `package.json` pointing at a host file was once copied into this networked workspace (`test_a_symlinked_manifest_is_refused_and_nothing_leaves_the_jail`).
     - *Lockfile required; `npm ci` only.* `npm ci` installs exactly what `package-lock.json` pins and fails rather than resolving. Without a lockfile the install is refused with a named reason and validation ends as "could not validate"; `npm install` is never run and versions are never resolved (`test_an_install_without_a_lockfile_is_refused`).
     - *No install scripts.* `--ignore-scripts` is on the argument array and `NPM_CONFIG_IGNORE_SCRIPTS=true` in its environment (`test_the_install_command_disables_scripts_and_pins_the_registry`).
     - *One registry.* `--registry=https://registry.npmjs.org/` is on the argument array. Before anything runs, the lockfile is parsed as JSON and every package must resolve to `https://registry.npmjs.org/` exactly — no other host, scheme, port or credentials — and carry an integrity hash; otherwise the install is refused (`test_a_lockfile_resolving_anywhere_else_is_refused`, `test_integrity_is_required_and_only_links_and_bundles_are_unresolved`).
     - *Labelled.* The validation report records where the dependency tree came from, and for an install: status, reason, the lockfile's SHA-256, its package count, the exact argument array and the exit code (`test_the_install_evidence_is_recorded`). A repository's own committed `node_modules` is never used.
     - *A refused or failed install never passes and never strands.* The run ends at `VALIDATION_FAILED` as "could not validate", with the record stored, and no pull request can follow (`test_a_repository_the_install_step_cannot_serve_is_not_validated`, in `tests/integration/test_pipeline_offline.py`).

     What this does not protect against, stated: a package that is malicious *as published* on the npm registry is installed; `--ignore-scripts` stops it running during install, but the validation commands then execute the application's code and its dependencies inside the network-denied sandbox. Integrity hashes pin the bytes the lockfile names; they do not vouch for them.
